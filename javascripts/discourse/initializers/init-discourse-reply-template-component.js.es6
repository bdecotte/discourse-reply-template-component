...
function openComposerWithTemplateAndAction(controller, post, wrap) {
  ...
  const postIdentifier = `${post.get('topic').id}-${post.id}-`;
    ...
    const replacers = [
      {
        ...
        fn: () => {
          ...
          return container.lookup("key-value-store:main").getObject(`${STORAGE_PREFIX}${placeholderIdentifier}`) || placeholderIdentifier;
        }
      },
      {
        regex: /(\=reponse=)/g,
